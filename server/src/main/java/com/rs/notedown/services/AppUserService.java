...
public class AppUserService {
  ...
  public AppUser getByUsername(String username) {
    return appUserRepository.findByUsername(username).orElseThrow(
        () -> new DataNotExistException("User", username));
  }

  public boolean isAlreadyExistForAdmin(String username, String email,
                                        String groupName) {
    return appUserRepository
        .findByUsernameOrEmailOrGroupName(username, email, groupName)
        .isPresent();
  }
  ...
  public boolean isSameGroup(long id) {
    AppUser currentUser = this.getByUsername(
