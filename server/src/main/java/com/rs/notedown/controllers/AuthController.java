...
public class AuthController {
  ...
  authenticateUser(@Valid @RequestBody SignInRequest signInRequest) {
    try {
  ...
  public ResponseEntity<Object> registerAdminUser(
      @Valid @RequestBody SignUpRequestForAdmin signUpRequestForAdmin) {
    try {
      if (appUserService.isAlreadyExistForAdmin(
              signUpRequestForAdmin.getUsername(),
              signUpRequestForAdmin.getEmail(),
              signUpRequestForAdmin.getGroupName())) {
        return new ApiErrorResponse(
            "Requested username or email or group name is already exist",
            HttpStatus.CONFLICT);
      }
      AppUser appUser =
          (AppUser)ModelMap.convert(signUpRequestForAdmin, AppUser.class);
