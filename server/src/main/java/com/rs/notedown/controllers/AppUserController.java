...
public class AppUserController {
    ...
    public ResponseEntity<Object> registerUser
            (@Valid @RequestBody(required = false) SignUpRequest signUpRequest, Principal principal) {
        try {
            if (appUserService.isAlreadyExistForUser(signUpRequest.getUsername(), signUpRequest.getEmail())) {
                return new ApiErrorResponse
                        ("Requested username or email is already exist", HttpStatus.CONFLICT);
            }
            ...
            appUser.setGroupName(appUserService.getByUsername(principal.getName()).getGroupName());
    ...
    public ResponseEntity<Object> getCurrentUser(Principal principal) {
        try {
            return new ApiSuccessResponse(new SanitizedAppUser(appUserService.getByUsername(principal.getName())));
        } catch (Exception e) {
            return new ApiErrorResponse(e.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    public ResponseEntity<Object> getUserById(@Valid @PathVariable long id) {
        try {
            return new ApiSuccessResponse(new SanitizedAppUser(appUserService.getById(id)));
        } catch (Exception e) {
            return new ApiErrorResponse(e.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    public ResponseEntity<Object> getAllUsersOfSameGroup(Principal principal) {
        try {
            List<SanitizedAppUser> userList = appUserService.getAllByGroupName
