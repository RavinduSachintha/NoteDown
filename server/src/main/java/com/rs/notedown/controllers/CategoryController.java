...
public class CategoryController {
    ...
    public ResponseEntity<Object> getCategoryById(@Valid @PathVariable long id) {
        try {
    ...
    public ResponseEntity<Object> getAllCategoriesOfMine(Principal principal) {
        try {
            return new ApiSuccessResponse
                    (categoryService.getAllByCreatedUser(appUserService.getByUsername(principal.getName()).getId()));
        } catch (Exception e) {
            return new ApiErrorResponse(e.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    public ResponseEntity<Object> getAllCategoriesOfGroupOfMine(Principal principal) {
        try {
    ...
    public ResponseEntity<Object> createCategory
            (@Valid @RequestBody CategoryCreateRequest categoryCreateRequest, Principal principal) {
        try {
            Category category = (Category) ModelMap.convert(categoryCreateRequest, Category.class);
    ...
    public ResponseEntity<Object> updateCategory(
            ...
            @Valid @RequestBody CategoryUpdateRequest categoryUpdateRequest, Principal principal) {
        try {
            Category storedCategory = categoryService.getById(id);
            Category category = (Category) ModelMap.convert(categoryUpdateRequest, Category.class);
            category.setId(id);
    ...
    @PreAuthorize("(hasRole('ADMIN') AND @categoryService.isSameGroup(#id)) OR " +
            "(hasRole('USER') AND @categoryService.isMine(#id))")
    public ResponseEntity<Object> deleteCategory(@Valid @PathVariable long id, Principal principal) {
        try {
